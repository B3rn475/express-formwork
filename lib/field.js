    ...
    this.toMiddleware = function () {
        ...
        registeredSteps.forEach(function (step) {
            ...
            case 'required':
                steps.push(function (req, res, next, end) {
                    if (req.form[name].value === '') {
                        req.form[name].error = step.error;
                        return end();
                    }
                    next();
                });
            ...
            case 'validator':
                steps.push(function (req, res, next, end) {
                    step.validator(req.form[name].value, function (err, error) {
                        if (err) {
                            return end(err);
                        }
                        if (error) {
                            req.form[name].error = error;
                            return end();
                        }
                        next();
                    }, req.form, req, res);
            ...
            case 'sanitizer':
                steps.push(function (req, res, next, end) {
                    step.sanitizer(req.form[name].value, function (err, value) {
                        if (err) {
                            return end(err);
                        }
                        req.form[name].value = value;
                        next();
                    }, req.form, req, res);
                });
                break;
