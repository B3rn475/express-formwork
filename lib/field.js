...
function Field(name, location) {
    ...
    if (typeof location !== 'string' && !util.isAfter(location)) {
    ...
    this.toMiddleware = function () {
        ...
        registeredSteps.forEach(function (step) {
            ...
            case 'required':
                steps.push(function (req, res, next, end) {
                    if (req.formwork[name].value === '') {
                        ...
                        req.formwork[name].error = step.error;
                        return end();
                    }
                    next();
                });
            ...
            case 'validator':
                steps.push(function (req, res, next, end) {
                    step.validator(req.formwork[name].value, function (err, error) {
                        if (err) {
                            return end(err);
                        }
                        if (error) {
                            req.formwork.isValid = false;
                            req.formwork[name].error = error;
                            return end();
                        }
                        next();
            ...
            case 'sanitizer':
                steps.push(function (req, res, next, end) {
                    step.sanitizer(req.formwork[name].value, function (err, value) {
                        if (err) {
                            return end(err);
                        }
                        req.formwork[name].value = value;
                        next();
                    }, req.formwork, req, res);
                });
                break;
